...
class CoreUpgrade extends Base
{
    ...
    public function checkingNewVersion(array $data): bool
    {
        ...
        if (!empty($data = $this->getAvailableVersionData()) && $notifiedVersion != $data['version']) {
            // create notification
            $this->notifyAboutNewVersion($data['version']);

            // update config
            $this->getConfig()->set('notifiedVersion', $data['version']);
            $this->getConfig()->save();
        }
    ...
    /**
     * Get available version data
     *
     * @return array
     */
    protected function getAvailableVersionData(): array
    {
        // prepare result
        $result = [];
