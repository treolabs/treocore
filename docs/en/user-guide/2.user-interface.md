...
<!-- [VT] дописать настройки для глобал серч, так как прочитав описание я решила проверить работу и не понятно было, что там ещещ что-то нужно настраивать -->
...
<!-- [VT] для Desktop Notifications, Progress Manager u Quick Create я бы на скрине выделила красным пункты в верхней строке, Саша обрезал последним новый пункт, но если выделить будет бросаться в глаза -->
### Progress Manager
...
- While you start typing TreoPIM determines automatically the search mask type of your search string -- available are Text, Number, Date and <!-- [VT] - не дописано -->
...
<!-- [VT] лучше добавить скрин на какой-то странице с выделенным блоком -->
## User Menu
...
- Per default your **user profile** is opened.
<!-- [VT] add screen -->
...
- It is also possible to generate a new password automatically.
...
<!-- [VT] add screen, highlight menu item -->
...
- The following actions are available for any entity per default: 
  ...
  - Merge - to merge the selected entity records <!--[VT] нет такого пункта в выпадающем списке, что мерджиться опиши и куда -->
...
- To follow the entity record please click on the button Follow, placed in the upper right corner of your Detail View.
...
<!-- [VT] add screen, выдели на скрине сам блок навигации -->
...
- Click on the Cancel button, if you changed your mind.
